...
并对 CBS / CBSFlow / CBSFlowBeam2 / CBSFlowBeam8 绘制 num_agents vs runtime 的散点图。
"""
...
def filter_and_prepare(df: pd.DataFrame, filters: Dict[str, object],
                       xcol: str = "num_agents", ycol: str = "run_time") -> pd.DataFrame:
...
def main():
    parser = argparse.ArgumentParser(description="绘制 num_agents vs runtime 散点图（按指定的 map_file 与 agent_file 筛选）")
    parser.add_argument(
        "--csv",
    ...
    parser.add_argument(
        "--out",
        default="",
        help="输出图片路径（留空则直接显示图窗，例如：plots/runtime_vs_agents.png）",
    )
    ...
    for name, extra_filters, _style in methods:
        ...
        sub = filter_and_prepare(df, filters, xcol="num_agents", ycol="run_time")
        if sub.empty:
            missing.append(name)
        else:
            prepared[name] = sub
